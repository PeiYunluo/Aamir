...
function loadWidget(config) {
  ...
  (function registerEventListener() {
    ...
    document.querySelector("#waifu-tool .fa-info-circle").addEventListener("click", () => {
      open("https://github.com/PeiYunluo");
    });
...
/*  function showHitokoto() {
    ...
    var baseurl = test.split('#')
    //windows.location.href = baseurl[0]
    open(baseurl[0]+"#/comment/complex-table");
  ...
  function showHitokoto() {
    ...
    //open(baseurl[0]+"#/comment/complex-table");
    // 增加 hitokoto.cn 的 API
    fetch("http://47.94.90.145:8090/misc/getOneComment")
      .then(response => response.json())
      .then(result => {
        var text = `这句一言来自 <span>「${result.data.content}」</span>，是 <span>${result.data.author}</span> 在文章<span>${result.data.postId}</span>投稿的。`;
        showMessage(result.hitokoto, 6000, 9);
        setTimeout(() => {
          showMessage(text, 4000, 9);
        }, 6000);
